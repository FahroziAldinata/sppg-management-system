...
- **Persistensi State Menu Detail ([ASUMSI/DISKUSI])**: Data MenuItem, MenuItemBahan, dan TargetGizi di frontend saat ini disimpan pada local-state dan hilang setelah refresh karena `GET /api/gizi/menu-harian` belum melakukan `include` relasi tersebut. Perlu keputusan apakah akan memodifikasi handler GET backend agar melakukan include relasi-relasi ini (disarankan) atau mempertahankan local-state.
