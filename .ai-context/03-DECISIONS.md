...
- **Approve PO 1-Tombol (FINAL, 2026-07-17)**: Aslap **tidak mengisi qty/kondisi granular per item ke sistem**. Pemeriksaan fisik barang dilakukan DI KERTAS di lapangan sesuai format `PEMERIKSAAN_BAHAN_P12`. Sistem: 1 tombol approve → status `DITERIMA` + isi `diterimaOlehId`/`diterimaAt`. Body request tidak diproses. Field `qtyDiterima` di schema tetap ada (nullable, YAGNI). Endpoint approve sempat terlanjur di-redesign granular lalu dikembalikan ke 1-tombol.

- **DATABASE_URL Runtime vs Migration (FINAL, 2026-07-17)**: Format connection string WAJIB dibedakan antara runtime dan migration:
  - **Runtime (Railway / production server)**: port `6543` (Supabase pooler) + `?pgbouncer=true` — koneksi transaksional via PgBouncer, prepared statement tidak diperlukan.
  - **Migration / schema engine (migrate deploy/reset/status lokal)**: port `5432` (Supabase direct connection) TANPA `?pgbouncer=true` — karena PgBouncer dalam mode transaction pool **tidak mendukung prepared statement** yang dipakai schema engine Prisma. Jika dipaksa lewat pooler, `prisma migrate deploy` akan gagal dengan error prepared statement.
